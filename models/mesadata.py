...
class MesaData:
    def __init__(self, history_file, only_read_header=False, read_data=True, read_data_cols=None,
    ...
    def trim_PMS(self):
        ...
        while count < 10:
            zamsix += 1
            if abs(logL[zamsix] - logLnuc[zamsix]) > 0.005:
                count = 0
            else:
                count += 1
        print(self.get('model_number')[zamsix])
...
def plot(mdata, x, y, *args, xscale=1, **kwargs):
    plt.plot(mdata.get(x)/xscale, mdata.get(y), *args, **kwargs)
    plt.xlabel('${{}}$'.format(x))
    plt.ylabel('${{}}$'.format(y))
...
def plot_profiles(profs, x, y, **kwargs):
    for i in range(len(profs)):
        plt.plot(profs[i].get(x), profs[i].get(y), c=plt.get_cmap('inferno')(i), **kwargs)
    plt.xlabel('${{}}$'.format(x))
    plt.ylabel('${{}}$'.format(y))
