...
module run_star_extras
   ...
   subroutine data_for_extra_history_columns(id, n, names, vals, ierr)
      ...
      if (b% s1% id == s% id) then
         ! check which star is which in the binary, this module works at the star level after all
         this_star = 1
         other_star = 2
      ...
      lq = log10(m2 / m1)
      vals(1) = eval_area(lq, s% r(1) / r_roche, ierr) * s% r(1) * s% r(1)
   ...
   subroutine my_fp_ft(id, nz, xm, r, rho, aw, ft, fp, r_polar, r_equatorial, report_ierr, ierr)
      ...
      real(dp) :: r_roche, lq, m1, m2, a, xtest, ytest, testval
      real(dp), dimension(:), allocatable :: ar
      include 'formats.inc'

      if (.not. inter_ok) then  ! setup ft, fp and irot interpolator and test it
         ! open data to interpolate
         write(*, *) 'starting interpolator setup'
         call setup_interpolator('../../fp_data.txt', xvals, num_xpts, yvals, num_ypts, fpfunc1d, ierr)
         call setup_interpolator('../../ft_data.txt', xvals, num_xpts, yvals, num_ypts, ftfunc1d, ierr)
         call setup_interpolator('../../irot_data.txt', xvals, num_xpts, yvals, num_ypts, irotfunc1d, ierr)
         call setup_interpolator('../../area_data.txt', xvals, num_xpts, yvals, num_ypts, afunc1d, ierr)
         xtest = -0.5
         ytest = 1.35
         ...
         call interp_evbipm_db(xtest, ytest, xvals, num_xpts, yvals, num_ypts, fpfunc1d, num_xpts, testval, ierr)
         write(*, *) 'fp   test gave', testval, 'should be close to 0.6'
         call interp_evbipm_db(xtest, ytest, xvals, num_xpts, yvals, num_ypts, ftfunc1d, num_xpts, testval, ierr)
         write(*, *) 'ft   test gave', testval, 'should be close to 1.0'
         call interp_evbipm_db(xtest, ytest, xvals, num_xpts, yvals, num_ypts, irotfunc1d, num_xpts, testval, ierr)
      ...
      if (r_roche <= 0) r_roche = binary_eval_rlobe(m1, m2, a)
      lq = log10(m2 / m1)
      allocate(ar(s% nz))  ! allocate space for different rs, so OMP doesn't trip over itself
      if (s% fitted_fp_ft_i_rot) then
         ...
         do j = 1, s% nz  ! for every cell, compute fp, ft from an interpolating table
            ar(j) = r(j) / r_roche
            ! if outside maximal tabulated r value, take an r value near edge of grid, should not happen often
            if (ar(j) > yvals(num_ypts)) ar(j) = yvals(num_ypts) - 1d-6
            ! call interpolators
            fp(j) = eval_fp(lq, ar(j), ierr)
            ft(j) = eval_ft(lq, ar(j), ierr)
...
!      write(*, *) ft(:)
!      write(*, *) ar(:)
      deallocate(ar)  ! free up the memory for this pass
   ...
   subroutine my_irot(id, ri, r00, ra, w_div_w_crit_roche, i_rot, di_rot_dlnr, di_rot_dw_div_wc)
      ...
      call binary_ptr(s% binary_id, b, ierr)
      if (ierr/=0) return
      if (b% s1% id == s% id) then  ! set which star is which
         this_star = 1
         other_star = 2
      else if (b% s2% id == s% id) then
         this_star = 2
      ...
      if (r_roche <= 0) then
         if (a <= 0) a = pow(standard_cgrav * (m1 + m2) * &
               pow((b% initial_period_in_days) * 86400, 2) / (4 * pi2), one_third)
         r_roche = binary_eval_rlobe(m1, m2, a)
      end if
      if (inter_ok) then
         ! Compute i_rot from an interpolating table
         ar = r00 / r_roche
         ! if outside maximal tabulated r value, take an r value near edge of grid, is unphysical regime
         if (ar > yvals(num_ypts)) ar = yvals(num_ypts) - 1d-6
         i_rot = eval_irot(log(m2/m1), ar, ierr)
!         write(*, *) r00, r_roche, i_rot, w_div_w_crit_roche  ! debug
   ...
   real(dp) function eval_area(lq, ar, ierr) result(area)
      real(dp), intent(in) :: lq, ar
      integer, intent(out) :: ierr
      call interp_evbipm_db(lq, ar, xvals, num_xpts, yvals, num_ypts, afunc1d, num_xpts, area, ierr)
      if(ierr/=0) then
         write(*, *) "error in eval area"
         return
      end if
   ...
   real(dp) function eval_fp(lq, ar, ierr) result(fp)
      real(dp), intent(in) :: lq, ar
      integer, intent(out) :: ierr
      call interp_evbipm_db(lq, ar, xvals, num_xpts, yvals, num_ypts, fpfunc1d, num_xpts, fp, ierr)
      if(ierr/=0) then
         write(*, *) "error in eval fp"
         return
      end if
   ...
   real(dp) function eval_ft(lq, ar, ierr) result(ft)
      real(dp), intent(in) :: lq, ar
      integer, intent(out) :: ierr
      call interp_evbipm_db(lq, ar, xvals, num_xpts, yvals, num_ypts, ftfunc1d, num_xpts, ft, ierr)
      if(ierr/=0) then
         write(*, *) "error in eval ft"
         return
      end if
   ...
   real(dp) function eval_irot(lq, ar, ierr) result(irot)
      real(dp), intent(in) :: lq, ar
      integer, intent(out) :: ierr
      call interp_evbipm_db(lq, ar, xvals, num_xpts, yvals, num_ypts, irotfunc1d, num_xpts, irot, ierr)
      if(ierr/=0) then
         write(*, *) "error in eval irot"
         return
      end if
   end function eval_irot


   subroutine setup_interpolator(filename, xvals, num_xpts, yvals, num_ypts, func1d, ierr)
      integer, intent(out) :: ierr
      character(LEN = *) :: filename
      integer :: k, iounit
      integer, intent(out) :: num_xpts, num_ypts
      real(dp), pointer :: xvals(:), yvals(:), func(:, :, :), func1d(:)

      write(*, *) 'loading ', filename
      ! open data to interpolate
      open(newunit = iounit, file = filename, status = 'old', action = 'read', iostat = ierr)

      read(iounit, *, iostat = ierr) num_xpts
      allocate(xvals(num_xpts))
      do k = 1, num_xpts
         read(iounit, *, iostat = ierr) xvals(k)
      end do

      read(iounit, *, iostat = ierr) num_ypts
      allocate(yvals(num_ypts))
      do k = 1, num_ypts
         read(iounit, *, iostat = ierr) yvals(k)
      end do

      ! create a 1d array with all the data, point func to it
      allocate(func1d(4 * num_xpts * num_ypts))
      func(1:4, 1:num_xpts, 1:num_ypts) => func1d(1:4 * num_xpts * num_ypts)
      do k = 1, num_xpts
      ...
      ! create interpolator
      call interp_mkbipm_db(xvals, num_xpts, yvals, num_ypts, func1d, num_xpts, ierr)
   ...
   subroutine my_atm(id, skip_partials, &
      ...
      real(dp) :: L_surf, R_surf, T_surf, P_surf, fp_surf, ft_surf, k_surf, &
            dr_p, dr_m, dA_dr, dfp_dr, dft_dr, A_surf, lq, r_roche, ar, m1, m2, a
      integer :: this_star=0, other_star=0
      include 'formats'
...
!      write(*, *) a / Rsun
      ...
      L_surf = s% L(1)
      R_surf = s% r(1)
!      write(*, *) 'R for BC', R_surf / Rsun
      ar = R_surf / r_roche
!      write(*, *) 'ar for BC', ar
      fp_surf = s% fp_rot(1)
      ft_surf = s% ft_rot(1)
      k_surf = s% opacity(1)
      A_surf = eval_area(lq, ar, ierr) * a * a
...
!      s% xtra(ix_omega) = Om_surf

      lnP_surf = log(P_surf)
      Teff = pow(L_surf / (boltz_sigma * A_surf), 0.25d0)
      T_surf = Teff
      lnT_surf = log(T_surf)
!      write(*, *) "BC values: R", R_surf, "L", L_surf, "A^1/4", (A_surf / (pi4 * R_surf*R_surf))**0.25, "P", P_surf, "T", T_surf, 'fp', fp_surf,&
!            "ft", ft_surf, "lq", lq
!      write(*, *) "area factor", eval_area(Om_surf)
!      write(*, *) "SB check", L_surf - A_surf*boltz_sigma*T_surf**4d0
      ...
      dr_m = 0.9999999d0
!      write(*, *) dr_p, dr_m
      dA_dr = (eval_area(lq, ar*dr_p, ierr) - eval_area(lq, ar*dr_m, ierr)) * R_surf / (dr_p - dr_m)
      dfp_dr = (eval_fp(lq, ar*dr_p, ierr) - eval_fp(lq, ar*dr_m, ierr)) / ((dr_p - dr_m) * R_surf)
      dft_dr = (eval_ft(lq, ar*dr_p, ierr) - eval_ft(lq, ar*dr_m, ierr)) / ((dr_p - dr_m) * R_surf)
