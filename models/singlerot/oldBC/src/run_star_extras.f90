...
module run_star_extras
   ...
   ! we store these two from the inlist, and use tighter controls in the HG
   real(dp) :: delta_HR_limit
   real(dp) :: delta_HR_hard_limit
   integer, parameter :: ilx_pre_ms = 1
   real(dp) :: starting_Omega, starting_omega_crit = 0.d0
   real(dp) :: crit_factor = 0.75
   integer :: count = 0
   ...
   subroutine extras_controls(id, ierr)
      ...
      ! use other atmosphere prescription
      if (s% use_other_surface_PT) then
         write(*, *) "using new boundary condition"
         s% other_surface_PT => my_atm
      else
         write(*, *) 'using default boundary condition'
      end if
   ...
   integer function extras_start_step(id)
     ...
     integer function extras_finish_step(id)
      ...
      if(starting_omega_crit <= 0) call determine_omega_crit(id, ierr)

      if (s% model_number > 1000 .and. s% lxtra(ilx_pre_ms)) then
         extras_finish_step = terminate
         write(*, *) "Terminate due to pre-MS evolution taking too long"
         return
      end if

      if (s% lxtra(ilx_pre_ms) .and. &
            abs(log10(abs(s% L_nuc_burn_total * Lsun / s% L(1)))) < 0.005) then
         if(count < 10) then
            count = count + 1
         else
            s% lxtra(ilx_pre_ms) = .false.
            s% scale_max_correction = 0.02d0
            s% ignore_species_in_max_correction = .true.
            write(*, *) "ZAMS reached, release rotation"
         end if
      end if
      if(s% lxtra(ilx_pre_ms)) then
         ! if here, still not in ZAMS, keep rotation fixed
         Omega_crit_rot = sqrt(standard_cgrav*s% m(1) / &
               pow(eval_equat_rad(crit_factor * starting_omega_crit) * s% r(1), 3))
         !$OMP PARALLEL DO PRIVATE(i) SCHEDULE(dynamic,2)
         do i = 1, s% nz
            s% j_rot(i) = crit_factor * starting_omega_crit * Omega_crit_rot * s% i_rot(i)
         end do
         !$OMP END PARALLEL DO
         if (abs(log10(abs(s% L_nuc_burn_total *Lsun /s% L(1)))) > 0.005) count = 0  ! reset count
      end if

!      write(*, *) "check SB law"
!      write(*, *) "teff", s% Teff
!      write(*, *) "L", s% L(1), "AsT_eff^4", eval_area(s% w_div_w_crit_roche(1)) * s% r(1) * s% r(1) * boltz_sigma * s% teff**4
!      write(*, *) "L", s% L(1), "AsT(1)^4", eval_area(s% w_div_w_crit_roche(1)) * s% r(1) * s% r(1) * boltz_sigma * s% T(1)**4


      if (s% center_h1 < 0.0001) then
         extras_finish_step = terminate
         write(*, *) "nearing H depletion. stopping"
      end if
   ...
   subroutine determine_omega_crit(id, ierr)
      integer, intent(in) :: id
      integer, intent(out) :: ierr
      type (star_info), pointer :: s
      real(dp) :: tol=1d-6, diff=1d0, const, hibound=1d0, lobound=0.d0, omega
      ierr = 0
      call star_ptr(id, s, ierr)
      write(*, *) 'omega determination'

      const = s% opacity(1) * s% L(1) / (pi4*clight*standard_cgrav*s% m(1))
      write(*, *) 'Gamma_edd', const
      if(eval(hibound) >= 0d0 .and. eval(lobound) >= 0d0) then  ! no eddington max spin
         omega = hibound
         diff=0d0
      end if
      do while(abs(diff) > tol)
         omega = 0.5 * (hibound + lobound)
         diff = eval(omega)
         if (diff < 0) then
            lobound = omega
         else
      ...
      starting_omega_crit = omega
      write(*, *) 'starting small omega', starting_omega_crit
      if(isNaN(starting_omega_crit)) stop "nan for omega"

      contains

      real(dp) function eval(om)
         real(dp) :: om
         eval = 1 - const * (1 - 0.07955242d0 * om ** 4 - 0.23215709d0 * om ** 6) / &
               (1 - two_thirds * om ** 2 + 0.21328774d0 * om ** 4 - 0.1068004d0 * om ** 6)
      end function eval

   end subroutine determine_omega_crit
...
!         write(*, *) r00, r_roche, i_rot, w_div_w_crit_roche  ! debug
   ...
   real(dp) function eval_equat_rad(om) result(r)
      real(dp), intent(in) :: om
      r = 1 + one_sixth * om ** 2.d0 - 0.00512375d0 * om ** 4 + 0.06562309d0 * om ** 6
...
!      P_surf = two_thirds*(standard_cgrav*m1/(k_surf*R_surf**2))
      ...
      lnP_surf = log(P_surf)
      T_surf = pow(L_surf / (boltz_sigma * A_surf), 0.25d0)
      lnT_surf = log(T_surf)
